...
OpticRender::OpticRender() {
	redirect = static_cast<MessageDisplay>(Preferences::find("optic_redirect", 0));
	sprite = NULL;
...
void OpticRender::displayText(const std::string& text) {
	switch(redirect) {
		case MessageDisplay::HUD_ONLY:
			HUDMessage(text);
			break;
		case MessageDisplay::CHAT_ONLY:
			Chat::medalText(text);
			break;
		case MessageDisplay::HUD_AND_CHAT:
			HUDMessage(text);
			Chat::medalText(text);
			break;
	}
